...
## Player Fork Node
##
## Groups a list of player dialogue options which are shown to the player in the same order as
## in this Node, expecting one to be selected by the player. Each option has an optional
## condition. Options with an untrue condition are tagged so the game can then gray them out,
## make them unselectable, or outright hide them (whichever approach is preferred).
